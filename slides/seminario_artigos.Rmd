...
date: 2021-02-18 .footnotesize[(última atualização `r Sys.Date()`)]
---

```{r, cache=FALSE, include=FALSE}
source("config/setup_knitr.R")
opts_chunk$set(fig.path = "figures/seminario_artigos")
library(RefManageR)
...
- Os artigos científicos devem ter sido publicados preferencialmente em
  **revistas científicas com avaliação por pares**.
  <!-- No entanto isso não exclui artigos publicados em anais de congresso, -->
  <!-- por exemplo (desde que não seja resumo) -->
]
...
Ler (e se basear) no artigo:

Keshav, S. (2007). [How to read a
paper](https://doi.org/10.1145/1273445.1273458). Computer Communication
Review, 37(3), 83–84.
]
...
- **Conclusão**: deve-se fazer uma interpretação pessoal sobre o
  **formato** do artigo
...
Não será necessário entregar texto ou slides nos seminários. A avaliação
será feita diretamente com base na apresentação.
]]
...
- Cada aluno terá 15 minutos para fazer a exposição oral
...
- Ao final de cada apresentação, um aluno será sorteado para comentar e
  fazer perguntas
...
<!-- - Os seminários serão apresentados no dia 05/11/2021 (ou 12/11/2021, -->
<!--   conforme o número de alunos) -->

```{r, include=FALSE, eval=FALSE}
## Para gerar o PDF
xaringanBuilder::build_pdf("slides/seminario_artigos.Rmd")
```
