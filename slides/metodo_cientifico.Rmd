...
- $[D|H_i]$ é a probabilidade do dado, condicionada à hipótese
  (**verossimilhança**)
- $[D]$ é a distribuição de probabilidade **a priori** para os dados
...
plot(y ~ x)
lines(x, predict(modA), col = 1, lwd = 2)
lines(x, predict(modB), col = 2, lwd = 2)
lines(x, predict(modC), col = 3, lwd = 2)
```
...
| D        | $C=aSe^{-bS}$       | Consumo aumenta e depois diminui com o aumento do tamanho do bando |
...
```{r, echo=FALSE, fig.show="hold", out.width="100%", fig.width=8, fig.height=6}
...
- Modelos B, C, e D seriem confrontados **individualmente** com o modelo
  nulo
- As hipóteses não podem (a princípio) serem testadas "par-a-par" pois
  isso diminui o nível de significância do teste
