...
opts_chunk$set(fig.path = "figures/mc/")
library(RefManageR)
...
- Se as duas premissas são verdadeiras, então a conclusão é verdadeira.
...
formal, em virtude de sua **lógica** (clássica).
...
- A partir de observações, é possível formular uma **hipótese
explicativa** da causa do fenômeno.

- O Método Indutivo, considera

  - As circunstâncias e a **frequência** com que ocorre determinado
    fenômeno
...
método indutivo ("problema da indução")
...
- Se esse experimento for realizado repetidas vezes, e o resultado
  ...
  **grau de confiança** (corroboração)
]
...
| Polanyi  | República da ciência                               | Múltiplas visões do mundo são permitidas de acordo com a opinião de diferentes cientistas. |
...
A **inferência bayesiana** (ou **estatística bayesiana**)
...
- As probabilidades são atribuídas apenas aos **conjuntos** de uma
  $\sigma$-algebra, e manipuladas por meio dos axiomas de Kolmogorov
  - Não é possível atribuir probabilidades para hipóteses
...
- A probabilidade é considerada como **extensão da lógica** indutiva, e
  manipuladas por meio dos desideratos de Pòlya e as algebras de RT Cox
  - As hipóteses naturalmente recebem probabilidades (a priori e a
  posteriori)
]
...
legend("topright", legend = LETTERS[1], col = 1, lty = 1, lwd = 2)
```
]
...
## lines(x, predict(modB), col = 3, lwd = 2)
## lines(x, predict(modC), col = 4, lwd = 2)
legend("topright", legend = LETTERS[1:2], col = 1:2, lty = 1, lwd = 2)
```
]
...
lines(x, predict(modB), col = 3, lwd = 2)
## lines(x, predict(modC), col = 4, lwd = 2)
legend("topright", legend = LETTERS[1:3], col = 1:3, lty = 1, lwd = 2)
```
]
